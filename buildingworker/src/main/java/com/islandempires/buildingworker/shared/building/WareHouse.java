...
import com.islandempires.buildingworker.shared.buildinglevelspec.WareHouseLevel;
...
public class WareHouse extends RawMaterialProductionStructures implements Serializable {
    ...
    public void islandBuildingLevelUpExecution(int nextLvl, String islandId) {
        ...
        try {
            ...
            increaseOrDecreaseIslandResourceFieldDTO.setIslandResourceEnum(IslandResourceEnum.RAW_MATERIAL_STORAGE_SIZE);

            int increaseOrDecreaseValue = (nextLvl > 1) ? wareHouseLevelList.get(nextLvl - 1).getStorageAmount() - wareHouseLevelList.get(nextLvl - 2).getStorageAmount() :
                    wareHouseLevelList.get(nextLvl - 1).getStorageAmount();
